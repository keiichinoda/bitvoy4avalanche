...
import { P2KeyGen, P2Signature, IP2KeyShare } from "@silencelaboratories/ecdsa-tss";
...
interface FrostWasm {
  // Ed25519 functions
  ed_dkg_round1: (party_id: number, max_signers: number, min_signers: number) => string;
  ed_dkg_round2: (secret_package_str: string, packages_str: string) => string;
  ed_dkg_round3: (secret_package_str: string, round1_packages_str: string, round2_packages_str: string) => string;
  ed_round1_commit: (key_package_str: string) => string;
  ed_build_signing_package: (message: Uint8Array, commitments_str: string) => string;
  ed_round2_sign: (
    key_package_str: string,
    nonces_str: string,
    signing_package_str: string
  ) => string;
  ed_aggregate_and_verify: (
    signing_package_str: string,
    signature_shares_map_str: string,
    pubkey_package_str: string
  ) => string;
  // secp256k1 functions
  secp_dkg_round1: (party_id: number, max_signers: number, min_signers: number) => string;
  secp_dkg_round2: (secret_package_str: string, packages_str: string) => string;
  secp_dkg_round3: (secret_package_str: string, round1_packages_str: string, round2_packages_str: string) => string;
  secp_round1_commit: (key_package_str: string) => string;
  secp_build_signing_package: (message: Uint8Array, commitments_str: string) => string;
  secp_round2_sign: (
    key_package_str: string,
    nonces_str: string,
    signing_package_str: string
  ) => string;
  secp_aggregate_and_verify: (
    signing_package_str: string,
    signature_shares_map_str: string,
    pubkey_package_str: string
  ) => string;
...
let frostWasm: FrostWasm | null = null;

async function initFrostWasm() {
  if (frostWasm) return frostWasm;
  ...
  try {
    ...
    const frostModule = require(absolutePath) as FrostWasm;
...
type Purpose = "ed_keygen" | "ed_sign" | "secp_keygen" | "secp_sign" | "ecdsa_tss_keygen" | "ecdsa_tss_sign";
type WSKind  = "P1_TO_P2" | "P2_TO_P1";

interface WSMessage {
  kind: WSKind;
  purpose: Purpose;
  path?: string;       // MPC-HD 用のパス
  sessionId: string;
  payload: string;   // FROSTのメッセージ（JSON文字列）
}
...
function parseMsg(raw: any): WSMessage {
  const text = typeof raw === "string" ? raw : Buffer.isBuffer(raw) ? raw.toString("utf8") : "";
  return JSON.parse(text) as WSMessage;
}

function formatCloseReason(message: string): string {
  if (!message) return "session error";
  const base = `session error: ${message}`;
  return base.length > 120 ? base.slice(0, 120) : base;
}

// ---- Ed25519 KeyGen（P2は保存する） ----
async function ed_handleKeygen(ws: any, sessionId: string, masterId: string, firstMessage?: WSMessage) {
  if (!frostWasm) throw new Error("frost-wasm not initialized");
  ...
  if (m1.purpose !== "ed_keygen" || m1.kind !== "P1_TO_P2") throw new Error("invalid round1");
  ...
  const round2Result = JSON.parse(
    frostWasm.ed_dkg_round2(p2Round1Secret, JSON.stringify(round2Packages))
  );
  ...
  if (m3.purpose !== "ed_keygen" || m3.kind !== "P1_TO_P2") throw new Error("invalid round3");
  ...
  if (!keyPackage || !publicKeyPackage) throw new Error("key_package or public_key_package missing");
...
async function ed_handleSign(ws: any, sessionId: string, masterId: string, messageHashHex?: string | null, firstMessage?: WSMessage) {
  if (!frostWasm) throw new Error("frost-wasm not initialized");
  ...
  if (!wallet) throw new Error(`wallet not found for masterId: ${masterId}`);
  ...
  if (m1.purpose !== "ed_sign" || m1.kind !== "P1_TO_P2") throw new Error("invalid round1");
  ...
  const messageBytes = new Uint8Array(
    messageHex.match(/.{2}/g)?.map(x => parseInt(x, 16)) || []
  );
  ...
  const signingPackage = JSON.parse(
    frostWasm.ed_build_signing_package(messageBytes, JSON.stringify(allCommitments))
  );
  ...
  const m3 = await waitOne(ws);
  console.log(`[P2] Received Round3 message: purpose=${m3.purpose}, kind=${m3.kind}`);
  if (m3.purpose !== "ed_sign" || m3.kind !== "P1_TO_P2") throw new Error("invalid round3");
...
async function secp_handleKeygen(ws: any, sessionId: string, masterId: string, firstMessage?: WSMessage) {
  if (!frostWasm) throw new Error("frost-wasm not initialized");
  ...
  if (m1.purpose !== "secp_keygen" || m1.kind !== "P1_TO_P2") throw new Error("invalid round1");
  const p1Round1Data = JSON.parse(m1.payload);
  // p1Round1Data.package はオブジェクトまたはJSON文字列の可能性がある
  const p1Round1PackageRaw = p1Round1Data.package;
  const p1Round1Package = typeof p1Round1PackageRaw === "string" ? JSON.parse(p1Round1PackageRaw) : p1Round1PackageRaw;
  ...
  console.log(`[P2] Round2 packages prepared: ${Object.keys(round2Packages).length} packages`);
  const round2Result = JSON.parse(
    frostWasm.secp_dkg_round2(p2Round1Secret, JSON.stringify(round2Packages))
  );
  const p2Round2Secret = round2Result.secret_package;
  // round2Result.package は JSON 文字列で、{ identifier: package_object } のマップ形式
  const p2Round2Package = JSON.parse(round2Result.package);
  ...
  const m3 = await waitOne(ws);
  console.log(`[P2] Received Round3 message: purpose=${m3.purpose}, kind=${m3.kind}`);
  if (m3.purpose !== "secp_keygen" || m3.kind !== "P1_TO_P2") throw new Error("invalid round3");
  const p1Round2Data = JSON.parse(m3.payload);
  const p1Round2PackageRaw = p1Round2Data.package;
  // p1Round2PackageRaw は { identifier: package_object } のマップ形式
  const p1Round2PackageMap = typeof p1Round2PackageRaw === "string" ? JSON.parse(p1Round2PackageRaw) : p1Round2PackageRaw;
...
async function secp_handleSign(ws: any, sessionId: string, masterId: string, messageHashHex?: string | null, firstMessage?: WSMessage) {
  if (!frostWasm) throw new Error("frost-wasm not initialized");
  ...
  if (m1.purpose !== "secp_sign" || m1.kind !== "P1_TO_P2") throw new Error("invalid round1");
  const p1RoundData = JSON.parse(m1.payload);
  // p1Commitmentsが文字列の場合はパースする
  const p1CommitmentsRaw = p1RoundData.commitments;
  const p1Commitments = typeof p1CommitmentsRaw === "string" ? JSON.parse(p1CommitmentsRaw) : p1CommitmentsRaw;
  ...
  const signingPackage = JSON.parse(
    frostWasm.secp_build_signing_package(messageBytes, JSON.stringify(allCommitments))
  );
  ...
  const m3 = await waitOne(ws);
  console.log(`[P2] Received Round3 message: purpose=${m3.purpose}, kind=${m3.kind}`);
  if (m3.purpose !== "secp_sign" || m3.kind !== "P1_TO_P2") throw new Error("invalid round3");
  const p1Round3Data = JSON.parse(m3.payload);
  const p1SignatureShare = p1Round3Data.signature_share;
...
async function ecdsa_tss_handleKeygen(ws: any, sessionId: string, masterId: string, firstMessage?: WSMessage) {
  ...
  if (m1.purpose !== "ecdsa_tss_keygen" || m1.kind !== "P1_TO_P2") throw new Error("invalid round1");
  ...
  const m3 = await waitOne(ws);
  console.log(`[P2] Received Round3 message: purpose=${m3.purpose}, kind=${m3.kind}`);
  if (m3.purpose !== "ecdsa_tss_keygen" || m3.kind !== "P1_TO_P2") throw new Error("invalid round3");
  console.log(`[P2] Processing Round3 message`);
  const r4 = await p2.processMessage(m3.payload);

  // p2_key_share と public_key を DB 保存
  const p2KeyShare = r4.p2_key_share as IP2KeyShare;
  if (!p2KeyShare?.public_key) throw new Error("p2_key_share missing");
...
async function ecdsa_tss_handleSign(ws: any, sessionId: string, masterId: string, messageHashHex?: string | null, firstMessage?: WSMessage) {
  ...
  if (m1.purpose !== "ecdsa_tss_sign" || m1.kind !== "P1_TO_P2") throw new Error("invalid round1");
  const msg1 = m1.payload;
  ...
  const p2MasterShare = wallet.p2_keyshare as IP2KeyShare;
  ...
  const m3 = await waitOne(ws);
  console.log(`[P2] Received Round3 message: purpose=${m3.purpose}, kind=${m3.kind}`);
  if (m3.purpose !== "ecdsa_tss_sign" || m3.kind !== "P1_TO_P2") throw new Error("invalid round3");
  console.log(`[P2] Processing Round3 message`);
  const r4 = await p2.processMessage(m3.payload);
...
function waitOne(ws: any, timeoutMs = 30000): Promise<WSMessage> {
  return new Promise((resolve, reject) => {
    const to = setTimeout(() => { cleanup(); reject(new Error("receive timeout")); }, timeoutMs);
    const onMsg = (raw: any) => { cleanup(); resolve(parseMsg(raw)); };
